...
public abstract class AbstractAnnotationBuilder<B extends AbstractAnnotationBuilder<B>> {
    ...
    public B add(final String name,
    ...
    public B addEnumAttribute(final String name,
        ...
        members.put(name, new FieldAccessExpr().target(new ClassOrInterfaceTypeExpr(annotationClassName)).field(new IdentifierExpr(valueName)));
        return (B) this;
    }
    ...
    public B addStringArray(final String name,
                            final List<String> values) {
        if (values != null && !values.isEmpty()) {
            final var literals = values.stream().map(v -> (Expr) new SimpleLiteralExpr(v)).toList();
    ...
    public B addStringArray(final String name,
        ...
        if (values != null && values.length > 0) {
            final var literals = Arrays.stream(values).map(v -> (Expr) new SimpleLiteralExpr(v)).toList();
            return addAttributeArray(name, literals);
        }
        return (B) this;
    }

    public <E extends Expr> B addAttributeArray(final String name,
                                                final List<E> values) {
        if (values != null && !values.isEmpty()) {
    ...
    public B addAttributeArray(final String name,
    ...
    public B addCompoundArray(final String name,
    ...
    public Annot build() {
        ...
        members.forEach((annot::attribute));
        return annot;
    }
}
